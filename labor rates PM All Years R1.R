...
project_manager <- labor_rate %>%
  filter(Position == "Project Manager") %>%                            # Filter for "Project Manager" only
...
project_manager$Year <- format(as.Date(project_manager$Year, format="%d/%m/%Y"),"%Y")
...
custom_levels <- c("Assistant", "General", "Senior", "Lead", "Associate")
...
print(n = 1000, project_manager_with_percentiles)
